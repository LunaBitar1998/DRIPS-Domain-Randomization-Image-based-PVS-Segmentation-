...
def predict(path_images,
    ...
    for i in range(len(path_images)):
        ...
        if compute[i]:
            # Preprocess image
            image, aff, h, im_res, shape, pad_idx, crop_idx, orig_res = preprocess(path_image=path_images[i],
                                                                                   n_levels=n_levels,
            ...
            #print(f"seg shape before resampling: {seg.shape}")
            #print(f"posterior shape before resampling: {posteriors.shape}")
            #print(f" preprocessing aff:{aff}")
            if target_res is not None and np.any((orig_res > target_res + 0.05) | (orig_res < target_res - 0.05)):
                # Load original image (for reference shape + affine)
                orig_nib = nib.load(path_images[i])
                orig_data = orig_nib.get_fdata()
                orig_aff = orig_nib.affine
                #print(f"original image affine:{orig_aff}")
                # Resample segmentation
                seg = edit_volumes.resample_volume_like(orig_data, orig_aff, seg, aff, interpolation='nearest')
                aff_seg = orig_aff
                utils.save_volume(seg, aff_seg, h, path_segmentations[i], dtype='int32')
                #print(f"seg affine:{aff_seg}")
                # Resample posterior(s)
                if path_posteriors[i] is not None:
                    if n_channels > 1:
                        posteriors = utils.add_axis(posteriors, axis=[0, -1])

                    resampled_posteriors = []
                    for c in posterior_class_to_save:
                        p_map = posteriors[..., c]
                        p_resampled = edit_volumes.resample_volume_like(
                            orig_data, orig_aff, p_map, aff, interpolation='linear')
                        resampled_posteriors.append(p_resampled)
                    aff_post=orig_aff
                    posteriors = np.stack(resampled_posteriors, axis=-1)
                    posteriors = np.squeeze(posteriors)
                    utils.save_volume(posteriors, aff_post, h, path_posteriors[i], dtype='float32')
                    #print(f"post affine:{aff_post}")
            else:
                ...
                if path_posteriors[i] is not None:
                    ...
                    if isinstance(posterior_class_to_save, int):
                        posteriors = posteriors[..., posterior_class_to_save]
                    else:
                        posteriors = np.stack([posteriors[..., c] for c in posterior_class_to_save], axis=-1)

                    posteriors = np.squeeze(posteriors)
                    utils.save_volume(posteriors, aff, h, path_posteriors[i], dtype='float32')

            print(f"seg shape after: {seg.shape}")
            print(f"posterior shape after: {posteriors.shape}")
...
def preprocess(path_image, n_levels, target_res, crop=None, min_pad=None, path_resample=None):
    ...
    return im, aff, h, im_res, shape, pad_idx, crop_idx, orig_res
